...
The working tree contains a finished implementation of a design that has since
been superseded, and must not be built from.

---

## ⚠ The working tree implements a design we have abandoned

`design.py`, `gen_sch.py` and `gen_pcb.py` currently implement a **±9 V
charge-pump supply with an onboard battery**. That work is complete and
internally consistent — `verify.py` reports the schematic matching the netlist
exactly, 58 nets and 322 pin connections — which is precisely what makes it
dangerous. Nothing about it looks wrong.

It is superseded. RMC's reply of 2026-07-30 established that the board takes
**bipolar power from the Poly-Drive II down the DIN-8**, so there is no
battery, no charge pump, and no power section at all.

Do not fabricate from the current `fab/` outputs. They describe the abandoned
board.

### What is in the tree, precisely

| File | State |
| --- | --- |
| `design.py` | ±9 V charge pump (TC1044S), 0402 passives, JST SH connectors, TSSOP-14 switches. Superseded in its supply section; the channel circuit is still broadly right. |
| `gen_sch.py` | Draws the above. Verified against `design.py`. |
| `gen_pcb.py` | Re-laid out to **56 × 94 mm**, 4 layers. Last measured **5 DRC violations**; one further fix was applied but **not verified** — treat the count as unconfirmed. |
| `verify.py` | Gained a `NO_CONNECT` mechanism so deliberately-floating pins are declared in `design.py` rather than waved through in the checker. **This change is worth keeping.** |
| `NOTES.md` | Describes the *original* 88 × 112 mm board. Stale throughout. |
| `ENCLOSURE.md` | Untracked. Tail-block enclosure sized around a Fishman battery pack. The mounting analysis holds; everything about the battery does not. |
| `fab/*` | Regenerated from the abandoned board. Stale. |

Nothing is committed. `HEAD` is still `8c425fb`, which is the original
hand-buildable board.
...
2. Decide what to do with the abandoned charge-pump work: commit it on a
   branch as a record of the ±9 V exploration, or discard it. It is currently
   uncommitted on `main`.
3. Rework `design.py` to the target design above, then `gen_sch.py`, then
   `gen_pcb.py`. Keep the `NO_CONNECT` mechanism in `verify.py`.
...
5. Revise `ENCLOSURE.md` — the mounting analysis stands; the battery bay,
   charging hatch and mass budget do not.
